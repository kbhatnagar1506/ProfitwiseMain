...
/** POST /api/context/merge — Merge company + financial context (OpenAI only). Call after fetching company and financial in separate requests. */
export async function POST(req: NextRequest) {
  ...
  let body: { companyContext?: string; financialContext?: FinancialContext }
  try {
    body = await req.json()
  } catch {
  ...
  const financialContext = body.financialContext as FinancialContext | undefined

  if (!financialContext || typeof financialContext.summary !== "string") {
    return NextResponse.json({ error: "financialContext required" }, { status: 400 })
  }
